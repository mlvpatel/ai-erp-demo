...
def main() -> int:
    ...
    required = {
        ...
        'resource "aws_ecs_task_definition" "ai_live_eval"': "private bounded OpenAI evaluation task",
    ...
    bootstrap_required = {
		...
		'permissions_boundary = aws_iam_policy.terraform_deploy_boundary.arn': "deployment permissions boundary",
		'variable = "token.actions.githubusercontent.com:repository"': "repository-scoped OIDC trust",
		'variable = "token.actions.githubusercontent.com:ref"': "main-branch-scoped OIDC trust",
		'variable = "aws:RequestedRegion"': "region-scoped deployment policy",
		'variable = "aws:ResourceAccount"': "account-scoped deployment policy",
		'variable = "aws:RequestTag/Project"': "tag-scoped deployment policy",
    ...
    if not image_workflow_path.is_file():
        ...
        workflow_markers = {
			...
			"Build immutable image locally for pre-promotion scanning": "scan-before-push ordering",
			...
			"cosign attest --yes --key": "SBOM and provenance attestation",
			"cosign verify-attestation --key": "attestation verification",
    ...
    if not delivery_workflow_path.is_file():
        ...
        delivery_markers = {
			...
			"FOUNDATION-AI-ERP-PILOT": "explicit foundation confirmation",
			"ACTIVATE-AI-ERP-PILOT": "explicit activation confirmation",
