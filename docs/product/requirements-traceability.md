...
| Create a project-specific folder structure, not a generic Claude/project structure. | Implemented | `README.md` layout, `AGENTS.md`, `config/repository-structure.json`, `docs/architecture/system-context-and-repository-map.md`, `docs/architecture/system-boundaries.md`, `docs/architecture/domain-data-model.md`, `apps/`, `services/`, `contracts/`, `infra/`, `tests/`. | `scripts/check-repository-structure.py` verifies required ERP-specific folders, implemented app markers, reserved app boundaries, and forbidden generic root directories. |
...
| Learn from the supplied skill/prompt structure and use it. | Implemented | `.agents/skills/ai-erp-delivery/SKILL.md`, `.agents/skills/erp-minimal-change/SKILL.md`, `AGENTS.md`. | Repository instructions require reading the skills before ERP feature or delivery work. |
| Incorporate the Ponytail minimal-change idea into the dev environment. | Implemented | `.agents/skills/erp-minimal-change/SKILL.md`. | The skill records the Ponytail-derived minimal-change ladder and upgrade-trigger rule. |
...
| Rehearse the service pilot without overstating acceptance. | Prepared with synthetic evidence only | `config/pilot-readiness.json`, `docs/runbooks/service-operations-synthetic-uat.md`, `docs/compliance/service-operations-pilot-evidence-template.md`. | `python3 scripts/check-pilot-readiness.py`; human, legal, deployment, restore, and capacity gates remain pending. |
